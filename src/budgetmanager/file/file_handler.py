...
class FileHandler:
    ...
    def create_directory(path: str) -> Path:
        """
        Creates a directory at config.DATA_ROOT/path (if `path` is relative)
        ...
        if candidate.is_absolute():
            ...
            dir_path = config.DATA_ROOT / candidate

        try:
            dir_path.mkdir(parents=True, exist_ok=True)
        except OSError as e:
            raise OSError(
                f"Failed to create directory '{dir_path}': {e.strerror}"
            ) from e
    ...
    def create_file(
        ...
        in the given directory. Relies on create_directory to
        ensure the directory exists.

        Args:
            directory_path (str): Relative (to DATA_ROOT)
                                  or absolute directory path.
            file_name (str): Name of the file without extension.
                             Allowed characters: letters, numbers,
                             hyphens, underscores.
            file_type (str): File extension (e.g., 'txt', 'csv').
        ...
        Raises:
            ...
            OSError: If the file or its parent directory cannot be created,
        ...
        try:
            ...
            raise OSError(
                "Cannot ensure directory exists for file "
                f"'{directory_path}': {e}"
            ) from e
        ...
        try:
            ...
            raise OSError(
                f"Failed to create file '{file_path}': {e.strerror}"
            ) from e
    ...
    def get_directory_path(*paths: str) -> Path:
        """
        Constructs a directory path by returning the first argument if
        it's absolute, otherwise joins components under DATA_ROOT.
    ...
    def get_file_path(*paths: str) -> Path:
        """
        Constructs a file path by returning the first argument if
        it's absolute, otherwise joins components under DATA_ROOT.
